...
let activeStream = null; // Güvenlik için mikrofon akışını takip edeceğimiz değişken
...
fileInput.addEventListener("change", (e) => {
    ...
    // Eğer yüklenen şey bir görsel ise (PNG dahil) sıkıştırarak işleme alıyoruz
    if (file.type.startsWith("image/")) {
        const reader = new FileReader();
        reader.onload = function(event) {
            const img = new Image();
            img.onload = function() {
                // Resmi küçültmek için sanal bir tuval (canvas) oluşturuyoruz
                const canvas = document.createElement("canvas");
                const MAX_WIDTH = 800; // Genişliği maksimum 800px yapıyoruz (Kaliteyi bozmaz, boyutu uçurur)
                ...
                // Resmi JPEG formatına çevirip %70 kalitede sıkıştırıyoruz (Firestore limitine takılmaz)
        ...
        // Görsel harici normal dosyalar (PDF vb.)
        const reader = new FileReader();
        reader.onload = function(event) {
...
// 4. Ultra Güvenli Ses Kayıt Sistemi (Girişte Dinleme Yapmaz, İş Bitince Donanımı Tamamen Kapatır)
async function startVoiceRecording() {
    try {
        // Butona basıldığı an mikrofon akışını açıyoruz
        ...
        mediaRecorder.onstop = () => {
            ...
            // GÜVENLİK KİLİDİ: Kayıt durduğu an mikrofon donanımının enerjisini kesiyoruz ve kapatıyoruz
            if (activeStream) {
...
// Ses Butonuna Tek Tıklama Kontrolü
...
function listenForMessages() {
    ...
    onSnapshot(q, (snapshot) => {
        ...
        snapshot.forEach((docSnap) => {
            ...
            // Mesaj İçeriği Gösterim Mantığı
            let contentBody = "";
